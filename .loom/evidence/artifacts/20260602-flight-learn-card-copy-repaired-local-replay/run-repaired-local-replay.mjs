...
const coreFields = new Set(["headline", "whatHappened", "whyItMatters", "expectedBehavior"]);
...
const candidates = [
  {
    ...
    priorEvidence: "evidence:20260602-flight-learn-small-model-batch-eval",
  },
  {
    ...
    expectedSha256: "8334b850b7bd46238c16b0c550df2138f0889bf433809008cc17a8b05761863e",
    priorEvidence: "evidence:20260602-flight-learn-small-model-batch-eval",
  },
  {
    ...
    expectedSha256: "72c5c3cb38fa32d5256e2fe30d03e7a64c6c79e668ad84057e3bd66e250b24fb",
    priorEvidence: "evidence:20260602-flight-learn-small-model-batch-eval",
  },
  {
    id: "phi4-mini-q4-k-m",
    ...
    expectedSha256: "88c00229914083cd112853aab84ed51b87bdf6b9ce42f532d8c85c7c63b1730a",
    priorEvidence: "evidence:20260602-flight-learn-small-model-batch-eval",
  },
];
...
function modelAuthoredFieldPresence(result) {
  const presence = {};
  for (const field of fields) {
    const value = result.view?.[field];
    const deterministic = result.deterministicView?.[field];
    presence[field] = Boolean(value) && value !== deterministic;
  }
  return presence;
}
...
function textSignalCategories(value) {
  ...
  if (/\.pi\/agent\/sessions|bearer\s+|api[_ -]?key|private key|\/users\/|\/home\//.test(normalized)) signals.push("privacy-leak-like");
  return signals;
}
function collectUnsafeAcceptedSignals(result) {
  if (!(result.usedLocalModel && result.fallbackReason === null)) return [];
  const values = fields
    .map((field) => ({ field, value: result.view?.[field], deterministic: result.deterministicView?.[field] }))
    .filter((item) => item.value && item.value !== item.deterministic);
  const signals = new Set();
  for (const item of values) for (const signal of textSignalCategories(item.value)) signals.add(signal);
  return [...signals];
}
function isGateEligibleCase(caseDef) { return !caseDef.coverage.includes("safety-adversarial"); }
function summarizeCase(candidate, caseDef, result, raw, latencyMs, promptLength, memorySample, providerError) {
  ...
  const safeProductGatePass = result.usedLocalModel && result.fallbackReason === null && unsafeAcceptedSignals.length === 0;
  const gateEligibleCase = isGateEligibleCase(caseDef);
  return {
    ...
    safeProductGatePass,
    gateEligibleSafeProductGatePass: gateEligibleCase && safeProductGatePass,
    ...
    modelAuthoredFieldPresence: modelAuthoredFieldPresence(result),
    modelAuthoredCoreFieldCount: Object.entries(modelAuthoredFieldPresence(result)).filter(([field, present]) => coreFields.has(field) && present).length,
    rawResponseLength: raw === null ? null : raw.length,
    rawResponseSha256: raw === null ? null : sha256String(raw),
    promptLength,
    latencyMs,
...
function aggregateCandidate(candidate, caseSummaries, extra = {}) {
  ...
  const modelAuthoredFieldCoverage = {};
  for (const field of fields) {
    ...
    modelAuthoredFieldCoverage[field] = caseSummaries.filter((item) => item.modelAuthoredFieldPresence[field]).length;
  }
  ...
  const gateEligibleSafePassCount = caseSummaries.filter((item) => item.gateEligibleSafeProductGatePass).length;
  const unsafeAcceptedCount = caseSummaries.filter((item) => item.unsafeAccepted).length;
  return {
    candidateId: candidate.id,
    label: candidate.label,
    totalCases: caseSummaries.length,
    gateEligibleCaseCount: caseSummaries.filter((item) => item.gateEligibleCase).length,
    ...
    gateEligibleSafeProductGatePassCount: gateEligibleSafePassCount,
    ...
    fieldCoverage,
    viewFieldCoverage,
    modelAuthoredFieldCoverage,
    latencyMs: numericSummary(latencies),
    promptLengthChars: numericSummary(prompts),
    memoryRssKb: numericSummary(rss),
    gatePass: gateEligibleSafePassCount >= 5 && unsafeAcceptedCount === 0,
    gateRule: "requires >=5 safe real product-gated model-enabled renders from gate-eligible non-safety/adversarial cases and zero unsafe/privacy accepted outputs",
    cases: caseSummaries,
    ...extra,
  };
}
...
function aggregateBatch(candidateReports, generatedAt) {
  ...
  const passingCandidates = ranReports.filter((report) => report.summary.gatePass && report.summary.unsafeAcceptedCount === 0);
  const unsafeAcceptedTotal = ranReports.reduce((sum, report) => sum + report.summary.unsafeAcceptedCount, 0);
  return {
    kind: "flight-learn-repaired-local-open-runtime-replay",
    generatedAt,
    ...
    candidateGateEligibleSafeProductGatePassCounts: Object.fromEntries(candidateReports.map((report) => [report.candidateId, report.summary?.gateEligibleSafeProductGatePassCount ?? 0])),
    ...
    unsafeAcceptedTotal,
    gateDisposition: passingCandidates.length > 0 ? "open-downstream-comprehension-validation" : "keep-downstream-comprehension-validation-blocked",
    gateRule: "unblock only if at least one local/open candidate reaches >=5 safe real product-gated renders from gate-eligible non-safety/adversarial cases, zero unsafe/privacy accepted outputs, and intact trust boundaries",
    hostedProviderUsed: false,
    loopbackOnly: true,
    runtimeInstallsOrUpgrades: false,
    modelDownloads: false,
    authorizedCachedModelsOnly: true,
    ...
    nonClaims: [
      ...
      "No product source, default configuration, model integration, hosted provider, or model download occurred.",
    ],
  };
}
...
function sha256String(value) { return createHash("sha256").update(value).digest("hex"); }
...
async function preflightCandidate(candidate) {
  ...
  if (!exists) {
    return { ok: false, reason: "cached-model-absent", cache: { status: "absent", displayPath: modelDisplayPath, expectedSha256: candidate.expectedSha256 }, modelPath: null };
  }
  const sizeBytes = await fileSizeBytes(modelPath);
  const sha256 = await sha256File(modelPath);
  const cache = { status: sha256 === candidate.expectedSha256 ? "verified" : "checksum-mismatch", displayPath: modelDisplayPath, sizeBytes, sizeGiB: sizeGiB(sizeBytes), sha256, expectedSha256: candidate.expectedSha256, downloadedNow: false };
  if (sha256 !== candidate.expectedSha256) return { ok: false, reason: "cached-model-checksum-mismatch", cache, modelPath: null };
  return { ok: true, reason: null, cache, modelPath };
}
...
async function evaluateCandidate(candidate, runtime, sourcePaths) {
  ...
  const baseProvenance = {
    candidateId: candidate.id,
    label: candidate.label,
    source: { sourceRepo: candidate.sourceRepo, sourceFile: candidate.sourceFile, upstreamModel: candidate.upstreamModel, licenseNote: candidate.licenseNote, priorEvidence: candidate.priorEvidence },
    cache: preflight.cache,
    runtime,
    authorization: "Already cached and authorized by evidence:20260602-flight-learn-small-model-batch-eval; no downloads or installs performed in this replay.",
  };
  if (!preflight.ok) {
  ...
  const stdoutPath = join(tmpdir(), `pfr-repaired-replay-${process.pid}-${candidate.id}-${port}.stdout.log`);
  const stderrPath = join(tmpdir(), `pfr-repaired-replay-${process.pid}-${candidate.id}-${port}.stderr.log`);
  const stdoutStream = createWriteStream(stdoutPath, { flags: "w" });
  const stderrStream = createWriteStream(stderrPath, { flags: "w" });
  const args = ["-m", preflight.modelPath, "--host", "127.0.0.1", "--port", String(port), "-c", "4096", "--no-webui", "--jinja"];
  ...
  try {
    ...
    for (const caseDef of cases) {
      ...
      const options = createLlamaCppLocalDiagnosisPolishOptions({ enabled: true, kind: "llama-cpp-server", baseUrl, model: candidate.modelLabel, timeoutMs: productTimeoutMs, maxOutputTokens });
      ...
      for (const width of widths) {
        const file = `repaired-${candidate.id}-${caseDef.id}-${width}.txt`;
        await writeFile(join(artifactDir, file), redactedRenderCase(caseDef, result, width));
        renderFiles.push(file);
      }
    }
    const summary = aggregateCandidate(candidate, caseSummaries, { model: baseProvenance.cache, source: baseProvenance.source, runtime: { path: runtime.path, versionStatus: runtime.versionStatus, versionStdoutPreview: runtime.versionStdoutPreview, versionStderrPreview: runtime.versionStderrPreview }, baseUrl, hostedProviderUsed: false, loopbackOnly: true, productTimeoutMs, maxOutputTokens, modelDisplayTextPersisted: false });
    const sourceAfter = await sourceFingerprints(sourcePaths);
...
async function artifactPrivacyScan(paths) {
  const patterns = [
    ["raw_home_path", new RegExp("/" + "Users" + "/[A-Za-z0-9._-]+|/home/[A-Za-z0-9._-]+", "i")],
    ["pi_session_path", new RegExp("\\.pi/agent/" + "sessions", "i")],
    ["private_key", new RegExp("-----BEGIN " + "[A-Z ]*" + "PRIVATE KEY-----", "i")],
    ["bearer", /Bearer\s+[A-Za-z0-9._~+/=-]+/i],
    ["secret_assignment", new RegExp("\\b(?:API_" + "KEY|TOKEN|SECRET|PASSWORD|PRIVATE_KEY)\\s*=", "i")],
  ...
  for (const path of paths) {
    ...
    if (rel.endsWith("run-repaired-local-replay.mjs")) continue;
...
async function main() {
  ...
  const sourceBeforeAll = await sourceFingerprints(sourcePaths);
  const reports = [];
  let stopReason = null;
  for (const candidate of candidates) {
    if (stopReason) {
      const sourceAfter = await sourceFingerprints(sourcePaths);
      reports.push({ candidateId: candidate.id, status: "skipped", reason: stopReason, summary: null, renderFiles: [], provenance: { candidateId: candidate.id, label: candidate.label, source: { sourceRepo: candidate.sourceRepo, sourceFile: candidate.sourceFile, upstreamModel: candidate.upstreamModel, licenseNote: candidate.licenseNote, priorEvidence: candidate.priorEvidence }, cache: { status: "not-checked-after-stop", displayPath: cacheDisplayPathFor(candidate), expectedSha256: candidate.expectedSha256 }, runtime }, listenerCheck: null, sourceSideEffectScan: compareSourceFingerprints(sourceAfter, sourceAfter) });
      continue;
    }
    ...
    if (report.summary?.unsafeAcceptedCount > 0) stopReason = `skipped-after-unsafe-accepted-in-${candidate.id}`;
    if (report.summary?.gatePass) stopReason = `skipped-not-needed-after-gate-pass-${candidate.id}`;
  }

  for (const report of reports) {
    if (report.status === "skipped") {
      await writeFile(join(artifactDir, `candidate-${report.candidateId}-summary.json`), `${JSON.stringify(report.summary, null, 2)}\n`);
      await writeFile(join(artifactDir, `candidate-${report.candidateId}-provenance.json`), `${JSON.stringify(report.provenance, null, 2)}\n`);
      await writeFile(join(artifactDir, `candidate-${report.candidateId}-source-side-effect-scan.json`), `${JSON.stringify(report.sourceSideEffectScan, null, 2)}\n`);
    }
  }
  ...
  const sourceAfterAll = await sourceFingerprints(sourcePaths);
  const finalSourceSideEffect = compareSourceFingerprints(sourceBeforeAll, sourceAfterAll);
  await writeFile(join(artifactDir, "01-replay-summary.json"), `${JSON.stringify(batchSummary, null, 2)}\n`);
  ...
  await writeFile(join(artifactDir, "06-source-side-effect-scan.json"), `${JSON.stringify({ pass: finalSourceSideEffect.pass && reports.every((report) => report.sourceSideEffectScan?.pass !== false), finalSourceSideEffect, perCandidate: Object.fromEntries(reports.map((report) => [report.candidateId, report.sourceSideEffectScan])) }, null, 2)}\n`);

  const artifactFilesForScan = [
    "01-replay-summary.json",
    "02-candidate-report-index.json",
    "03-render-line-widths.json",
    "04-render-contract-check.json",
    "06-source-side-effect-scan.json",
    ...
    ...reports.map((report) => `listener-${report.candidateId}.txt`).filter((file) => reports.some((report) => report.listenerCheck?.file === file)),
    ...allRenderFiles,
  ...
  const index = {
    ...
    candidates: candidates.map((candidate) => ({ candidateId: candidate.id, label: candidate.label, sourceRepo: candidate.sourceRepo, sourceFile: candidate.sourceFile, upstreamModel: candidate.upstreamModel, licenseNote: candidate.licenseNote, cacheDisplayPath: cacheDisplayPathFor(candidate), expectedSha256: candidate.expectedSha256, priorEvidence: candidate.priorEvidence })),
    artifacts: { buildOutput: "00-build.txt", replaySummary: "01-replay-summary.json", candidateIndex: "02-candidate-report-index.json", widthChecks: "03-render-line-widths.json", hiddenInternals: "04-render-contract-check.json", privacyScan: "05-artifact-privacy-scan.json", sourceSideEffect: "06-source-side-effect-scan.json", evidenceDossier: "../../20260602-flight-learn-card-copy-repaired-local-replay.md" },
    renderFiles: allRenderFiles,
    checks: { widthPass: widthChecks.pass, hiddenInternalsPass: internalsCheck.pass, privacyPass: privacy.pass, sourceSideEffectPass: finalSourceSideEffect.pass && reports.every((report) => report.sourceSideEffectScan?.pass !== false) },
  ...
  const output = {
    ...
    candidateGateEligibleSafeProductGatePassCounts: batchSummary.candidateGateEligibleSafeProductGatePassCounts,
    candidateUnsafeAcceptedCounts: batchSummary.candidateUnsafeAcceptedCounts,
    passingCandidates: batchSummary.passingCandidates,
    gateDisposition: batchSummary.gateDisposition,
    stopReason,
    widthPass: widthChecks.pass,
    hiddenInternalsPass: internalsCheck.pass,
    privacyPass: privacy.pass,
    sourceSideEffectPass: finalSourceSideEffect.pass && reports.every((report) => report.sourceSideEffectScan?.pass !== false),
    artifactDir: ".loom/evidence/artifacts/20260602-flight-learn-card-copy-repaired-local-replay",
  };
  await writeFile(join(artifactDir, "07-run-output.json"), `${JSON.stringify(output, null, 2)}\n`);
