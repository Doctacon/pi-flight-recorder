...
- `spec:flight-learn-inbox-ux` - owns `/flight-learn` UX behavior; amended with REQ-033 through REQ-048 and SCN-011 through SCN-015 to state that model-enabled comprehension can be the intended rich path, local drafts can be displayed as non-authoritative reading help, local LLM card copy should cover the primary reading surface, and deterministic fallback remains safe and non-dead-ending.
- `plan:20260531-flight-learn-llm-authored-card-copy` - successor repair branch created after operator screenshot feedback showed the current focused card still exposes too much detector/provenance language.
- `evidence:20260531-flight-learn-llm-card-copy-operator-feedback` - records the screenshot feedback that `Raw clue`/`Why suggested`/evidence-adjacent copy should become local-LLM-authored display copy while evidence remains hidden by default.
...
Active, but the comprehension validation branch has looped back to UX/model repair. `ticket:20260529-flight-learn-constrained-json-adapter` is closed with evidence and clear follow-up audit. `ticket:20260529-flight-learn-constrained-judge-replay` closed as a negative accepted-narrative gate. `ticket:20260529-flight-learn-local-draft-comprehension-gate` is closed with evidence and clear follow-up audit, adding the explicitly labeled local LLM draft reading-help path behind existing local-model opt-in and hard display gates. `ticket:20260529-flight-learn-comprehension-validation` produced a render/review packet but is now blocked after operator screenshot feedback showed the current card still exposes too much deterministic detector/provenance language.

The next executable path is `plan:20260531-flight-learn-llm-authored-card-copy`, starting with `ticket:20260531-flight-learn-llm-card-copy-contract`. `ticket:20260529-flight-learn-model-comprehension-integration` remains blocked as accepted-narrative integration. `ticket:20260529-flight-learn-dogfood-corpus-outcomes` remains blocked until the successor comprehension validation passes.
...
- 2026-05-31: Operator screenshot feedback showed the current focused card still exposes too much detector/provenance language and should instead use local-LLM-authored primary card copy while keeping evidence hidden by default. Added `evidence:20260531-flight-learn-llm-card-copy-operator-feedback`, amended `spec:flight-learn-inbox-ux` with REQ-042 through REQ-048 and SCN-013 through SCN-015, and created repair plan `plan:20260531-flight-learn-llm-authored-card-copy`. Dogfood corpus/outcome collection stays blocked.
