...
Implementation complete at unit-test level. `src/pi-extension.ts` registers `/flight-sync`, `/seen-this-before`, and `flight_seen_this_before`, wrapping the core library rather than duplicating behavior. Fake-Pi wrapper tests pass. Live Pi TUI loading has not been manually verified. Evidence is recorded in `evidence:20260522-mvp-validation`. Ready for closure review with live-Pi residual risk explicit.
